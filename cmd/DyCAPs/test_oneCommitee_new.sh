...
echo "protocol start"
./DyCAPS_NOVSS_NOLog -n $COUNTER -f $FAULT -op1 2 -op2 client -mp metadata -lp list&

# start threads representing nodes in the Commitee
for i in `seq 0 $(($COUNTER-1))`;
do
  ./DyCAPS_NOVSS_NOLog -n $COUNTER -f $FAULT -op1 2 -op2 onlyOneCommitee -id $i -mp metadata -lp list&
