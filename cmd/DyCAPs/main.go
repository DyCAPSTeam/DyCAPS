...
func main() {
	...
	if *option1 == "1" {
		...
		case "newCommitee":
			...
			fmt.Fprintf(f, "ProactivizeTotal, %d\n", p.ProactivizeEnd.Sub(p.ProactivizeStart).Nanoseconds())
			fmt.Fprintf(f, "Init,%d\n", p.InitEnd.Sub(p.InitStart).Nanoseconds())
			fmt.Fprintf(f, "Commit, %d\n", p.CommitEnd.Sub(p.CommitStart).Nanoseconds())
			fmt.Fprintf(f, "Verify, %d\n", p.VerifyEnd.Sub(p.VerifyStart).Nanoseconds())
			fmt.Fprintf(f, "Reshare, %d\n", p.ReshareEnd.Sub(p.ReshareStart).Nanoseconds())
			fmt.Fprintf(f, "Vote, %d\n", p.VoteEnd.Sub(p.VoteStart).Nanoseconds())
			fmt.Fprintf(f, "Recover, %d\n", p.RecoverEnd.Sub(p.RecoverStart).Nanoseconds())
			fmt.Fprintf(f, "MVBA, %d\n", p.MVBAEnd.Sub(p.MVBAStart).Nanoseconds())
			fmt.Fprintf(f, "Refresh, %d\n", p.RefreshEnd.Sub(p.RefreshStart).Nanoseconds())
