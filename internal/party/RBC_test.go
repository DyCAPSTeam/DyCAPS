...
func TestRBC(t *testing.T) {
	...
	go func() {
		p[0].RBCSender(&protobuf.Message{Type: "Test", Sender: uint32(0), Id: ID, Data: []byte("helloworld")}, ID)
	}()
...
func TestMultiRBC(t *testing.T) {
	...
	for i := uint32(0); i < N; i++ { // there is one malicious node,who doesn't send any Message
		for j := uint32(0); j < N; j++ {
			go func(i uint32, j uint32) {
				m := p[j].RBCReceiver([]byte(string(ID) + strconv.Itoa(int(i))))
	...
	for i := uint32(0); i < N; i++ {
		go func(i uint32) {
			p[i].RBCSender(&protobuf.Message{Type: "Test", Sender: uint32(i), Id: ID, Data: []byte("helloworld by " + strconv.Itoa(int(i)))}, []byte(string(ID)+strconv.Itoa(int(i))))
		}(i)
	}
