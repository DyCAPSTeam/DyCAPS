...
func (p *HonestParty) InterpolateComOrWit(degree uint32, targetExpIndex uint32, List []bls.G1Point) bls.G1Point {
	CWList := make([]bls.G1Point, degree+1)
	copy(CWList, List)

	// degree=2t
	if targetExpIndex > 0 && targetExpIndex < degree+1 {
		return CWList[targetExpIndex-1]
	}

	if targetExpIndex >= 0 && targetExpIndex <= 3*degree+1 {
		return bls.LinCombG1(List)
	}
